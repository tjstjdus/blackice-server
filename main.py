...
def build_realtime_weather_dataset(target_time_str, max_points=None, debug=True):
    ...
    if debug:
        print("[3] ASOS 매칭 후:", terrain.shape)
    ...
    if debug:
        print("[6] weather_df:", weather_df.shape)
...
def add_prediction_columns(df, debug=True):
    ...
    if debug:
        print("[8] 결빙모델 입력 컬럼:", icing_features)
        print("[9] 블랙아이스모델 입력 컬럼:", blackice_features)
...
def predict_realtime_blackice(
    ...
    if debug:
        print("[10] 최종 결과:", result_df.shape)
